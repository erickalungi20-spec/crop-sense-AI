...
let gpsCoords = null;  // { lat, lon } once "Use my location" succeeds

// ── GPS location button ──────────────────────────────────────────────────────
gpsBtn.addEventListener('click', () => {
  if (!navigator.geolocation) {
    setGpsStatus('error', 'Geolocation is not supported on this device/browser.');
    return;
  }
  setGpsStatus('pending', 'Getting your location…');
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      try {
        ...
        setGpsStatus('success', `✓ Using: ${data.place_name} `);
        addClearGpsLink();
      } catch {
        setGpsStatus('success', `✓ Location captured (${gpsCoords.lat.toFixed(4)}, ${gpsCoords.lon.toFixed(4)}) `);
        addClearGpsLink();
      }
      // GPS overrides manual district/county/etc — grey them out so it's
      // clear which location will actually be used.
      document.getElementById('district-block').classList.add('superseded');
      document.querySelectorAll('.refine-field').forEach(el => el.classList.add('superseded'));
      gpsBtn.disabled = false;
    },
    (err) => {
      gpsCoords = null;
      const msg = err.code === err.PERMISSION_DENIED
        ? 'Location permission denied. Enter your district manually below.'
        : 'Could not get your location. Enter your district manually below.';
...
function setGpsStatus(kind, msg) {
  gpsStatus.hidden = false;
  gpsStatus.className = `gps-status ${kind}`;
  gpsStatus.textContent = msg;
}

function addClearGpsLink() {
  const clearBtn = document.createElement('button');
  clearBtn.type = 'button';
  clearBtn.textContent = 'Clear';
  clearBtn.className = 'gps-clear-btn';
  clearBtn.addEventListener('click', () => {
    gpsCoords = null;
    gpsStatus.hidden = true;
    document.getElementById('district-block').classList.remove('superseded');
    document.querySelectorAll('.refine-field').forEach(el => el.classList.remove('superseded'));
  });
  gpsStatus.appendChild(clearBtn);
}
...
async function runAdvisory() {
  ...
  const sensor_ph = document.getElementById('sensor_ph').value;
  const sensor_moisture = document.getElementById('sensor_moisture').value;
  const sensor_temperature = document.getElementById('sensor_temperature').value;

  const body = {
    crop_key, area_acres, purpose, planting_date, phone,
    sensor_ph, sensor_moisture, sensor_temperature,
  };
  ...
  if (gpsCoords) {
    ...
    body.district = districtSel.value;
    body.county = document.getElementById('county').value.trim();
    body.subcounty = document.getElementById('subcounty').value.trim();
    body.village = document.getElementById('village').value.trim();
    if (!body.district) {
      showError('Select a district, or use "Use my current location" above.');
      return;
    }
  }
  ...
  // If the farmer opted in and gave a phone number, register the plot
  // for check-in monitoring (uses the same location/crop fields already
  // submitted above).
  const wantsSubscribe = document.getElementById('subscribe-checkin').checked;
  if (wantsSubscribe && phone) {
    try {
    ...
    } catch { /* non-critical — advisory result still shown either way */ }
  }
}
...
function renderAdvisory(data) {
  ...
  if (data.used_sensor_reading) {
    const label = data.sensor_source === 'manual' ? 'your entered soil reading' : 'your ESP32 soil sensor';
    sourceNote = `<div class="source-note sensor">✓ Boosted by ${label}</div>`;
  } else if (data.location_precision === 'district') {
    sourceNote = `<div class="source-note">District-level location used — add county/subcounty/village or "Use my location" for a more precise plot-level reading</div>`;
    ...
    sourceNote = `<div class="source-note">Live weather + soil data · ${data.location_precision}-level precision</div>`;
  }

  resultEl.innerHTML = `
    <div class="result-header">
      <span class="result-crop">${data.crop}</span>
      <span class="result-district">${data.location_label}</span>
    </div>
